...
describe('EtherToken', () => {
    ...
    describe('deposit', () => {
        it('should throw if caller attempts to deposit more Ether than caller balance', async () => {
            ...
            const ethToemit Deposit = initEthBalance.plus(1);

            return expectInsufficientFundsAsync(etherToken.deposit.sendTransactionAsync({ value: ethToemit Deposit }));
        });
        ...
        it('should convert deposited Ether to wrapped Ether tokens', async () => {
            ...
            const ethToemit Deposit = new BigNumber(Web3Wrapper.toWei(new BigNumber(1)));

            const txHash = await etherToken.deposit.sendTransactionAsync({ value: ethToemit Deposit });
            ...
            expect(finalEthBalance).to.be.bignumber.equal(initEthBalance.minus(ethToemit Deposit.plus(ethSpentOnGas)));
            expect(finalEthTokenBalance).to.be.bignumber.equal(initEthTokenBalance.plus(ethToemit Deposit));
        });
    });
    ...
    describe('withdraw', () => {
        ...
        it('should convert ether tokens to ether with sufficient balance', async () => {
            ...
            await web3Wrapper.awaitTransactionSuccessAsync(
                await etherToken.deposit.sendTransactionAsync({ value: ethToemit Deposit }),
                constants.AWAIT_TRANSACTION_MINED_MS,
            );
            const initEthTokenBalance = await etherToken.balanceOf.callAsync(account);
            const initEthBalance = await web3Wrapper.getBalanceInWeiAsync(account);
    ...
    describe('fallback', () => {
        it('should convert sent ether to ether tokens', async () => {
            ...
            const ethToemit Deposit = Web3Wrapper.toBaseUnitAmount(new BigNumber(1), 18);

            const txHash = await web3Wrapper.sendTransactionAsync({
                from: account,
                to: etherToken.address,
                value: ethToemit Deposit,
                gasPrice,
            });
