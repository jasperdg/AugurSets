...
export class TestFixture {
    ...
    public async approveCentralAuthority(): Promise<void> {
        ...
        console.log("Augur address: ", authority.address);
        console.log("Controller address: ", this.contractDeployer.controller.address);
    ...
    public async buyCompleteSets(market: Market, amount: BN): Promise<void> {
        ...
        const claimTradingProceeds = await this.contractDeployer.getContract("ClaimTradingProceeds")
        const timeContract = await this.contractDeployer.getContract("TimeControlled");

        const numTicks = await market.getNumTicks_();
        const ethValue = amount.mul(numTicks);

        console.log("CompleteSets address: ", completeSets.address);
        console.log("claimTradingProceeds address: ", claimTradingProceeds.address);
        console.log("TimeControlled address: ", timeContract.address);
